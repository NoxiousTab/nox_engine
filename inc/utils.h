...
constexpr std::string_view ver = "dev 1.0.0";

constexpr int mate_eval = 32000;
constexpr int mate_threshold = mate_eval - 1000;
constexpr int no_eval = -32001;
constexpr int neg_inf = -32001;
constexpr int pos_inf = 32001;

const int max_depth = 128;
const int max_move_cnt = 256;

template <std::size_t N>
constexpr double sum_array(const std::array<double, N>& arr, std::size_t i = 0, double acc = 0.0) {
    return i == N ? acc : sum_array<N>(arr, i + 1, acc + arr[i]);
}

constexpr int PawnNormalization = 283;
...
//static_assert(static_cast<int>(std::reduce(as.begin(), as.end())) == PawnNormalization);

static_assert(static_cast<int>(sum_array(as)) == PawnNormalization, "Sum does not match PawnNormalization");

std::tuple<int, int, int> getWDL(const int score, const int ply);
int toCentiPawns(const int score, const int ply);
 
static inline bool scoreISMATE(const int score) {
    return std::abs(score) > mate_threshold;
}

static inline bool scoreISWINNINGMATE(const int score) {
    return score > mate_threshold;
}

static inline bool scoreISLOSINGMATE(const int score) {
    return score < -mate_threshold;
}

static inline int scoreLOSINGMATE(const int level) {
    return level + mate_eval;
}

namespace move_flag {
    constexpr ui8 none = 0;
    constexpr ui8 castleShort = 1;
    constexpr ui8 castleLong = 2;
    constexpr ui8 promoteToKnight = 3;
    constexpr ui8 promoteToBishop = 4;
    constexpr ui8 promoteToRook = 5;
    constexpr ui8 promoteToQueen = 6;
    constexpr ui8 enPassantPossible = 7;
    constexpr ui8 enPassantDone = 8;
}
...
namespace Color {
    constexpr ui8 None = 0;
    constexpr ui8 White = 1;
    constexpr ui8 Black = 2;
}

namespace Type {
    constexpr ui8 None = 0;
    constexpr ui8 Pawn = 1;
    constexpr ui8 Knight = 2;
    constexpr ui8 Bishop = 3;
    constexpr ui8 Rook = 4;
    constexpr ui8 Queen = 5;
    constexpr ui8 King = 6;
}

namespace Piece {
    constexpr uint8_t wPieceOffset = 0;
        constexpr uint8_t bPieceOffset = 8;
        constexpr uint8_t None = 0;
        constexpr uint8_t WhitePawn = wPieceOffset + Type::Pawn;
        constexpr uint8_t WhiteKnight = wPieceOffset + Type::Knight;
        constexpr uint8_t WhiteBishop = wPieceOffset + Type::Bishop;
        constexpr uint8_t WhiteRook = wPieceOffset + Type::Rook;
        constexpr uint8_t WhiteQueen = wPieceOffset + Type::Queen;
        constexpr uint8_t WhiteKing = wPieceOffset + Type::King;
        constexpr uint8_t BlackPawn = bPieceOffset + Type::Pawn;
        constexpr uint8_t BlackKnight = bPieceOffset + Type::Knight;
        constexpr uint8_t BlackBishop = bPieceOffset + Type::Bishop;
        constexpr uint8_t BlackRook = bPieceOffset + Type::Rook;
        constexpr uint8_t BlackQueen = bPieceOffset + Type::Queen;
        constexpr uint8_t BlackKing = bPieceOffset + Type::King;
}

constexpr bool isValidPiece(const ui8 piece) {
    return (piece >= 1 && piece < 7) || (piece >= 9 && piece < 15);
};

constexpr bool isValidPieceOrNone(const ui8 piece) {
    return piece == Piece::None || isValidPiece(piece);
};

constexpr bool isNotPawn(const ui8 piece) {
    return (piece != Piece::WhitePawn && piece != Piece::BlackPawn);
};

constexpr uint64_t WhiteSquares = 0b0101010110101010010101011010101001010101101010100101010110101010;
constexpr uint64_t BlackSquares = 0b1010101001010101101010100101010110101010010101011010101001010101;
...
constexpr void setBitTrue(ui64 &n, ui8 where) {
    n |= 1ull << where;
}

constexpr void setBitFalse(ui64 &n, ui8 where) {
    n &= ~(1ull << where);
}

constexpr bool checkBit(const ui64 &n, const ui8 where) {
    return (n >> where)&1ull;
}

constexpr int popCount(const ui64 &n) {
    return std::popcount(n);
}

constexpr int lzCount(const ui64 &n) {
    return std::countl_zero(n);
}

constexpr int popSquare(ui64 &n) {
    const int temp = std::countr_zero(n);
    n &= (n - 1);
    return temp;
}

constexpr int lsbSquare(const ui64 n) {
    return static_cast<int>(std::countr_zero(n));
}

constexpr int msbSquare(const ui64 n) {
    return static_cast<int>(63 - std::countl_zero(n));
}

constexpr ui8 getRank(const int sq) {
    return sq >> 3;
}

const ui8 getFile(const int sq) {
    return (sq & 0b000111);
}

constexpr ui8 typeOfPiece(const ui8 piece) {
    return (piece & 0b0111);
}

constexpr ui8 colorOfPiece(const ui8 piece) {
    return (piece > 0) + (piece > 8);
}

constexpr ui8 sideToColor(const bool sid) {
    if(sid == side::White) {
        return Color::White;
    } else {
        return Color::Black;
...
constexpr ui8 Square(const ui8 rank, const ui8 file) {
    return rank*8 + file;
}

constexpr ui8 Mirror(const ui8 sq) {
    return (sq ^ 0b111000);
}

constexpr ui64 SquareBit(const ui8 sq) {
    return 1ull<<sq;
}

constexpr ui64 murmurHash3(ui64 key) {
    key ^= key >> 33;
...
bool startsWith(const std::string &first, const std::string &second);
std::vector<std::string> split(const std::string &command);
void debugBoard(const ui64 bits);
...
namespace console {
    ...
    const std::string contrast = "\x1b[30;103m";

    inline void clrScreen() {
        std::cout << "\033[2J\033[1;1H" << std::flush;
    };

    inline std::string formatNum(const ui64 n) {
        struct punct : std::numpunct<char> {
            char do_thousands_sep() const override { return ','; }
            std::string do_grouping() const override { return "\3"; }
        };
        std::locale formatLocale(std::locale::classic(), new punct);
        std::stringstream ss;
...
static inline ui8 squareToNum(const std::string &sq) {
    const ui8 file = sq[0] - 'a';
    const ui8 rank = sq[1] - '1';
...
struct staticVector {
    ...
    inline T &operator[](const std::size_t ind) {
        assert(ind < cnt);
        return memo[ind];
    }
    ...
    inline auto end() const {
        return (memo.begin() + static_cast<std::ptrdiff_t>(cnt));
    }
    ...
    protected:
        std::array<T, cap> memo{};
        std::size_t cnt = 0;
};


constexpr std::array<ui64, 64> kingMoveBits = {
...
constexpr std::array<ui64, 64> knightMoveBits = {
...
constexpr std::array<ui64, 64> whitePawnAttacks = {
...
constexpr std::array<ui64, 64> blackPawnAttacks = {
        0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000,
...
struct zobristHashes {
        MultiArray<ui64, 15, 64> PieceSquare;
        std::array<ui64, 4> Castling;
        std::array<ui64, 8> EnPassant;
        ui64 SideToMove;
};

static constexpr zobristHashes zobrist = {
        ...
        .EnPassant = {{
                0x70CC73D90BC26E24, 0xE21A6B35DF0C3AD7, 0x003A93D8B2806962, 0x1C99DED33CB890A1, 0xCF3145DE0ADD4289, 0xD0E4427A5514FB72, 0x77C621CC9FB3A483, 0x67A34DAC4356550B,
        }},

        .SideToMove = 0xF8D626AAAF278509,
